...
export function detectWinOrLost(playerEntity: PlayerEntity, state: State): [boolean, boolean] {
...
export function handlePlayerSpeed(playerEntity: PlayerEntity, state: State): PlayerEntity {
  ...
  return {
    ...playerEntity, speedX, speedY, jumpCount, lastJump,
  };
}



function handlePlayerCollision(entity: PlayerEntity, otherEntity: Entity, originalEntity: PlayerEntity): PlayerEntity {
...
export function handlePlayerCollisions(entity: PlayerEntity, state: State): PlayerEntity {
  const moved: PlayerEntity = {
    ...entity, x: entity.x + entity.speedX, y: entity.y + entity.speedY,
  };

  const otherEntities : Entity[]= state.entities.filter(
    (other) => other.id !== entity.id && other.type !== "door" && other.type !== "trap",
  ).concat(staticEntities(state.window));


  return otherEntities
    .reduce((result: PlayerEntity, other: Entity) => handlePlayerCollision(result, other, entity), moved) as PlayerEntity;
}

export function normalizePosition(entity: PlayerEntity, state: State): PlayerEntity {
  const { window } = state;
  return {
    ...entity,
...
export function updatePlayerEntity(entity: PlayerEntity, state: State) {
  const funcs :((playerEntity: PlayerEntity, state: State) => PlayerEntity)[] =
    [handlePlayerSpeed, handlePlayerCollisions, normalizePosition];
